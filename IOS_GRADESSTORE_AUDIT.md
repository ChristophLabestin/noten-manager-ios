...
### 17) Legacy cleanup deletes course docs but not subcollections (low/medium)
**Issue**
- The debug cleanup helper `cleanupLegacyTopLevelCourses(deleteLegacy:)` deletes legacy course documents but does not remove their `exams`/`homeworks` subcollections.

**Evidence**
- Legacy cleanup deletes only the document reference when `deleteLegacy` is true. `noten-manager-ios/noten-manager-ios/Stores/GradesStore.swift:2879-2893`
- Subcollections are only deleted in `deleteCourse`/`deleteClass` via `deleteCourseSubcollections`. `noten-manager-ios/noten-manager-ios/Stores/GradesStore.swift:6721-6748`

**Impact / likely symptoms**
- Orphaned legacy subcollection data remains in Firestore after cleanup, increasing storage and potentially reappearing if a legacy doc is recreated.

**Recommendation**
- When deleting legacy course docs via cleanup, also delete subcollections (reuse `deleteCourseSubcollections`).

**Status**
- ✅ Fixed: cleanup now deletes legacy subcollections before removing the legacy course doc.

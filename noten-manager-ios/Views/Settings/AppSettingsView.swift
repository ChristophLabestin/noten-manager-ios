...
struct AppSettingsView: View {
    ...
    @State private var isReAuthenticated: Bool = false
    ...
    @State private var showSupportHistorySheet: Bool = false
    ...
    private func resetReAuthState() {
        isReAuthenticated = false
    }
    
    private func performReAuth() {
        // Trigger re-auth flow based on provider
        guard let provider = externalAuthProvider else { return }
        Task {
            if provider == "Apple" {
                // We need the window for ASAuthorizationController
                guard let scene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
                      let window = scene.windows.first else { return }
                
                let result = await authManager.signInWithApple(presentationAnchor: window)
                if case .success = result {
                    isReAuthenticated = true
                }
            } else if provider == "Google" {
                 // We need top view controller for GIDSignIn
                 guard let scene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
                       let rootParam = scene.windows.first?.rootViewController else { return }
                 
                 let result = await authManager.signInWithGoogle(presenting: rootParam)
                 if case .success = result {
                     isReAuthenticated = true
                 }
            }
        ...
        NavigationStack {
            ScrollViewReader { proxy in
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 18) {
                        ...
                        helpCard
                            .softFadeIn(enabled: animationsOn, delay: 0.11, offset: 12)
                        schoolYearCard
            ...
            .sheet(isPresented: $showResetAccountSheet) {
                ResetConfirmSheet(
                    ...
                    isExternalAuth: isExternalAuthAccount,
                    onRequestReAuth: performReAuth,
                    isReAuthenticated: $isReAuthenticated
                )
            }
            .onChange(of: showResetAccountSheet) { _, show in
                if !show { resetReAuthState() }
            }
            ...
            .onChange(of: showDeleteAccountSheet) { _, show in
                if !show { resetReAuthState() }
            }
    ...
    private var notificationCard: some View {
        SettingsCard(
            ...
            subtitle: "Erinnerungen & Support",
            systemImage: "bell.badge.fill",
            accent: .green
        ) {
            NavigationLink(destination: NotificationSettingsView().environmentObject(store)) {
                SettingsButtonLabel(
                    title: "Einstellungen verwalten",
                    subtitle: "Tägliche Übersicht, Support-Updates",
                    icon: "bell.fill",
                    accent: .green
                )
            }
    ...
    private var helpCard: some View {
        SettingsCard(
            title: "Hilfe",
            subtitle: "Anleitungen & Support",
            systemImage: "questionmark.circle.fill",
            accent: .indigo
        ) {
            VStack(spacing: 16) {
                 NavigationLink(destination: HelpCenterView().environmentObject(store)) {
                     SettingsButtonLabel(
                 ...
                 Button {
                     ...
                     SettingsButtonLabel(
                        ...
                        subtitle: "Freigabe für Support anfordern",
                 ...
                 Button {
                     ...
                     SettingsButtonLabel(
                        title: "Support-Verlauf",
                        subtitle: "Deine Anfragen & Tickets",
    ...
    private var generalCard: some View {
        SettingsCard(
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                private func confirmResetAccount() {
                    Task {
                        ...
                        // Security check for Social Auth
                        if isExternalAuthAccount && !isReAuthenticated {
                            // If user is external auth (Apple/Google), they don't have a password.
                            // We use biometrics (if available/enabled) as an additional security layer here.
                            if biometricManager.biometricsAvailable && biometricManager.isEnabledForActiveUser {
                                let bioSuccess = await biometricManager.authenticate(reason: "Account-Reset bestätigen")
                                if !bioSuccess {
                                    // User cancelled or failed biometrics
                                    return
                                }
                            } else {
                                // If biometrics not available, we rely on the manual re-auth flow which sets isReAuthenticated = true
                                // If they haven't done that yet, they shouldn't be here (button disabled).
                                // Double check:
                                guard isReAuthenticated else {
                                    resetAccountError = "Bitte verifiziere dich zuerst."
                                    return
                                }
                            }
                        }
                        ...
                        do {
                            let pwd: String? = isExternalAuthAccount ? nil : resetAccountPassword
                            try await store.resetEntireAccount(password: pwd)
                            ...
                            if nsError.code == 17014 || nsError.localizedDescription.contains("requiresRecentLogin") {
                                resetAccountError = "Sicherheitsprüfung fehlgeschlagen. Bitte melde dich einmal ab und wieder an."
                            } else {
                ...
                private func confirmDeleteAccount() {
                    Task {
                        ...
                        if isExternalAuthAccount && !isReAuthenticated {
                            if biometricManager.biometricsAvailable && biometricManager.isEnabledForActiveUser {
                                let bioSuccess = await biometricManager.authenticate(reason: "Account-Löschung bestätigen")
                                if !bioSuccess { return }
                            } else {
                                guard isReAuthenticated else {
                                    deleteError = "Bitte verifiziere dich zuerst."
                                    return
                                }
                            }
                        }
                        ...
                        do {
                            let pwd: String? = isExternalAuthAccount ? nil : deleteAccountPassword
                            try await store.deleteAccountCompletely(password: pwd)
                            ...
                            let nsError = error as NSError
                            if nsError.code == 17014 || nsError.localizedDescription.contains("requiresRecentLogin") {
                                deleteError = "Zu lange inaktiv. Bitte melde dich zu deiner Sicherheit einmal ab und wieder an, bevor du den Account löschst."
                            } else {
                    ...
                    NavigationStack {
                        ...
                        .sheet(isPresented: $showResetSheet) {
                            ResetConfirmSheet(
                                ...
                                isReAuthenticated: .constant(false)
                            )
                        }
            ...
            private struct ResetConfirmSheet: View {
                ...
                // Extra params for social auth support
                var isExternalAuth: Bool = false
                var onRequestReAuth: (() -> Void)? = nil
                ...
                private var biometricsAvailable: Bool {
                    biometricManager.biometricsAvailable && biometricManager.isEnabledForActiveUser
                }
                
                private var readyToConfirm: Bool {
                    if isProcessing { return false }
                    guard slideDone else { return false }
                    
                    if isExternalAuth {
                        // For external auth:
                        // User MUST be verified (via Biometrics OR Manual Re-Auth) before proceeding.
                        ...
                        ScrollView(showsIndicators: false) {
                            VStack(spacing: 20) {
                                ...
                                if !isExternalAuth {
                                    SettingsCard(
                                        title: "Verifizierung",
                                        subtitle: "Bestätige mit deinem Passwort.",
                                    ...
                                    SettingsCard(
                                        ...
                                        subtitle: "Sicherheitsprüfung via Provider",
                                        ...
                                        if isReAuthenticated {
                                            Button {} label: {
                                                HStack(spacing: 10) {
                                                    ZStack {
                                                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                                                            ...
                                                            .overlay(
                                                                RoundedRectangle(cornerRadius: 10, style: .continuous)
                                                                    .stroke(Color.green.opacity(colorScheme == .dark ? 0.30 : 0.20), lineWidth: 1)
                                                            )
                                                    ...
                                                    VStack(alignment: .leading, spacing: 2) {
                                                        Text("Erfolgreich verifiziert")
                                                            .font(.body.weight(.bold))
                                                            .foregroundStyle(.primary)
                                                        Text("Du kannst den Vorgang abschließen.")
                                                            .font(.caption)
                                                            .foregroundStyle(.secondary)
                                                    }
                                                ...
                                                .padding(.horizontal, 10)
                                                .padding(.vertical, 8)
                                                .background(GradeCardStyle.surface(colorScheme: colorScheme, theme: store.theme, accent: .green, cornerRadius: 16))
                                                .overlay(GradeCardStyle.border(colorScheme: colorScheme, accent: .green, cornerRadius: 16))
                                                .shadow(
                                            ...
                                            Button {
                                                Task {
                                                    let success = await biometricManager.authenticate(reason: "Identität bestätigen")
                                                    if success {
                                                        isReAuthenticated = true
                                                    }
                                                }
                                                ...
                                                HStack(spacing: 10) {
                                                    ZStack {
                                                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                                                            ...
                                                            .overlay(
                                                                RoundedRectangle(cornerRadius: 10, style: .continuous)
                                                                    .stroke(Color.blue.opacity(colorScheme == .dark ? 0.30 : 0.20), lineWidth: 1)
                                                            )
                                                    ...
                                                    VStack(alignment: .leading, spacing: 2) {
                                                        Text("Mit \(biometricManager.biometryName()) bestätigen")
                                                            .font(.body.weight(.bold))
                                                            .foregroundStyle(.primary)
                                                        Text("Erforderlich zum Fortfahren")
                                                            .font(.caption)
                                                            .foregroundStyle(.secondary)
                                                    }
                                                ...
                                                .padding(.horizontal, 10)
                                                .padding(.vertical, 8)
                                                .background(GradeCardStyle.surface(colorScheme: colorScheme, theme: store.theme, accent: .blue, cornerRadius: 16))
                                                .overlay(GradeCardStyle.border(colorScheme: colorScheme, accent: .blue, cornerRadius: 16))
                                                .shadow(
                                                    color: Color.black.opacity(colorScheme == .dark ? 0.40 : 0.06),
                                                    radius: 4,
                                                    x: 0,
                    ...
                    GeometryReader { geo in
                        ...
                        ZStack(alignment: .leading) {
                            ...
                            HStack {
                                Spacer()
                                VStack(spacing: 6) {
                                    Text(isConfirmed ? "Bestätigt – Passwort eingeben" : "Zum Bestätigen schieben")
                                        .font(.footnote.weight(.semibold))
